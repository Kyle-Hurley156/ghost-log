...
export const GhostChefModal = ({ isOpen, onClose, targets, currentTotals, setToast, aiCooldown, setAiCooldown }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-gray-900 w-full max-w-sm rounded-2xl p-6 border border-gray-800/50 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-black tracking-tight text-white flex items-center gap-2"><ChefHat className="accent-text" size={22}/> GHOST CHEF</h3>
        ...
        <div className="bg-black/50 p-4 rounded-xl mb-4 text-center border border-gray-800/50">
          <p className="text-gray-500 text-[10px] uppercase font-bold tracking-widest">Budget Remaining</p>
          <div className="flex justify-center gap-6 mt-2">
            <div><span className="text-2xl font-black text-white">{reqCals}</span> <span className="text-[10px] text-gray-500">kcal</span></div>
            <div><span className="text-2xl font-black accent-text">{reqProt}g</span> <span className="text-[10px] text-gray-500">prot</span></div>
          </div>
        </div>
        {!suggestion ? (
          <button onClick={handleGetSuggestion} disabled={loading || aiCooldown > 0} className="w-full accent-bg hover:opacity-90 disabled:bg-gray-800 disabled:text-gray-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-[0.98]">
          ...
          <div className="space-y-3 animate-in slide-in-from-bottom-4">
            <div className="bg-black/30 p-4 rounded-xl border border-gray-800/50">
              ...
              <p className="text-xs accent-text italic mb-3">{suggestion.reason}</p>
              <ul className="text-sm text-gray-300 list-disc pl-4 space-y-1 mb-3">{suggestion.ingredients.map((ing, i) => <li key={i}>{ing}</li>)}</ul>
              <div className="flex gap-3 text-xs font-mono border-t border-gray-800/50 pt-2">
            ...
            <button onClick={handleGetSuggestion} disabled={loading || aiCooldown > 0} className="w-full bg-gray-800 disabled:bg-gray-900 disabled:text-gray-600 text-gray-300 py-3 rounded-xl font-bold text-xs transition-all active:scale-[0.98]">
